    ...
    >
      <div v-for="post in postList" :key="post.id" class="w-full">
        <article class="flex-1 xl:p-12 p-6 my-4 rounded bg-green-600 shadow-md">
          <p class="text-sm text-white animated fadeIn">
            {{ post.created_at }}
          </p>
          <h1 class="text-4xl font-bold text-white animated fadeIn">
            {{ post.title }}
          </h1>
          <p class="text-base py-4 animated fadeIn">{{ post.preview }} ...</p>
          <div class="flex animated fadeIn delay-1s">
            <div
